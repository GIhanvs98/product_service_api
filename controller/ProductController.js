...
const findAllProduct = async (request, response) => {
    try{
        ...
        return response.status(200).json({code: 200, message: 'product data data...', data:{list:productList, dataCount:productListCount}});
    }catch (e) {
        response.status(500).json({code: 500, message: 'product went wrong...', error: e});
    }


}
module.exports = {
    createProduct, updateProduct, deleteProduct, findProductById, findAllProduct
}